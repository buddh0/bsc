...
const (
	...
	highestVerifiedHeaderChanSize = 10 // highestVerifiedHeaderChanSize is the size of channel listening to HighestVerifiedHeaderEvent.
)
...
type VotePool struct {
	...
	highestVerifiedHeaderCh  chan core.HighestVerifiedHeaderEvent
	highestVerifiedHeaderSub event.Subscription
...
func NewVotePool(chain *core.BlockChain, engine consensus.PoSA) *VotePool {
	votePool := &VotePool{
		...
		curVotesPq:              &votesPriorityQueue{},
		futureVotesPq:           &votesPriorityQueue{},
		highestVerifiedHeaderCh: make(chan core.HighestVerifiedHeaderEvent, highestVerifiedHeaderChanSize),
		votesCh:                 make(chan *types.VoteEnvelope, voteBufferForPut),
		engine:                  engine,
	}

	// Subscribe events from blockchain and start the main event loop.
	votePool.highestVerifiedHeaderSub = votePool.chain.SubscribeHighestVerifiedHeaderEvent(votePool.highestVerifiedHeaderCh)
...
func (pool *VotePool) loop() {
	defer pool.highestVerifiedHeaderSub.Unsubscribe()

	for {
		select {
		// Handle ChainHeadEvent.
		case ev := <-pool.highestVerifiedHeaderCh:
			if ev.Header != nil {
				latestBlockNumber := ev.Header.Number.Uint64()
				pool.prune(latestBlockNumber)
				pool.transferVotesFromFutureToCur(ev.Header)
			}
		case <-pool.highestVerifiedHeaderSub.Err():
			return
...
func (pool *VotePool) putIntoVotePool(vote *types.VoteEnvelope) bool {
	...
	voteBlock := pool.chain.GetVerifiedHeaderByHash(targetHash)
	if voteBlock == nil {
	...
	if !isFutureVote {
		...
		log.Trace("The vote info to send is:", "voteBlockNumber", targetNumber, "voteBlockHash", targetHash)
...
func (pool *VotePool) transferVotesFromFutureToCur(latestBlockHeader *types.Header) {
	...
	for futurePq.Len() > 0 && futurePq.Peek().TargetNumber <= latestBlockNumber {
		blockHash := futurePq.Peek().TargetHash
		header := pool.chain.GetVerifiedHeaderByHash(blockHash)
		if header == nil {
